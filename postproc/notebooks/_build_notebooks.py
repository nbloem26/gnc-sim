...
nb01 = notebook([
    ...
    code(BOOTSTRAP),
    code("from gncpost.loaders import load_run\n"
...
nb02 = notebook([
    ...
    code("import pandas as pd\n"
         ...
         "df"),
    code("assert all(r.passed for r in results), 'a validation check failed'\n"
         "print('All analytic validation checks PASSED')"),
    md("## USSA76 density & temperature profile\n"
       "The Python twin of the C++ atmosphere, overlaid with the published reference points "
       "used in the table check."),
    code("from gncpost.atmosphere import ussa76, REFERENCE_TABLE\n\n"
    ...
    md("## Terminal-velocity convergence\n"
...
# ---------------------------------------------------------------------------
nb03 = notebook([
    md("# 03 — Monte-Carlo Miss Distance & CEP\n\n"
    ...
    md("## Miss-distance distribution + CEP\n"
    ...
    md("## Tie to the characterized sensor noise\n"
    ...
    code("imu = json.loads((REPO / 'configs' / 'sensor_params.json').read_text())['imu']\n"
...
def main():
    for name, nb in (("01_single_run", nb01), ("02_validation", nb02),
