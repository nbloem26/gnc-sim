...
def _write_fake_run(tmp_path):
    ...
    for name, cols in (
        ...
        ("gnc.csv", "t,accel_cmd_x,accel_cmd_y,accel_cmd_z,fin_x,fin_y,fin_z,"
                    "los_angle,los_rate,v_closing,range,nav_x,nav_y,nav_z"),
        ("sensors.csv", "t,imu_accel_true_x,imu_accel_meas_x,imu_gyro_true_x,"
    ...
    (tmp_path / "manifest.json").write_text(json.dumps({
        "scenario": "t", "model": "3dof", "seed": 1, "intercept": True,
        "miss_distance": 1.23, "intercept_time": 4.5, "num_frames": 2,
    }))
...
def test_load_summary(tmp_path):
    df = pd.DataFrame({
        "case": [0, 1], "seed": [1, 2], "miss_distance": [0.5, 1.5],
        "intercept_time": [4.0, 4.2], "intercept": [1, 1],
    })
