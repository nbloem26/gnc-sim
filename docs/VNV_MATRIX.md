...
| Sensor | `radar_pheno` (`trackers[].type`) | Range-equation SNR + Swerling RCS + clutter/ECM → CA-CFAR detection: Pfa held on noise-only, Pd matches closed form at SNR, RCS statistics match the Swerling case | `phenomenology_test::Cfar.*`, `phenomenology_test::Swerling.*`, `phenomenology_test::RadarSnr.*` | — | `phenomenology.py` ROC/Pd-vs-Pfa sweep |
| Sensor | `ir_pheno` (`trackers[].type`) | NETD + Beer-Lambert atmospheric transmission → contrast SNR → angular noise, CA-CFAR-gated detection | `phenomenology_test::IrSnr.NetdAndAtmosphereMapToAngularNoise`, `phenomenology_test::Phenomenology.DetectionIsDeterministicForFixedSeed` | — | `phenomenology.py` ROC/Pd-vs-Pfa sweep |
| Tracking | `jpda` (`trackers.association.mode`) | JPDA gates+weights detections so the true track survives decoys/clutter (purity high, degrades monotonically with clutter density); M-of-N lifecycle confirms/deletes; Covariance-Intersection track-to-track fusion lowers covariance with complementary inputs and stays consistent (non-optimistic); deterministic | `data_association_test::Jpda.*`, `data_association_test::TrackLifecycleTest.*`, `data_association_test::CovarianceIntersection.*`, `data_association_test::JpdaRunner.*` | — | `fusion.py` track-purity-vs-clutter sweep |
