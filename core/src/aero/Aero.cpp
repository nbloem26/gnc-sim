...
Vector3 AeroModel::dragForce(const Vector3& vel_world, const AtmSample& atm) const {
  const double speed_mps = vel_world.norm();
  if (speed_mps <= kMinSpeed) return Vector3{};
  ...
  const Vector3 v_hat = vel_world / speed_mps;
  const double drag_mag_n = 0.5 * atm.density * speed_mps * speed_mps * cd * cfg_.ref_area;
  return v_hat * (-drag_mag_n);
}
...
Vector3 AeroModel::force6dof(const Vector3& vel_world, const Quaternion& att,
  ...
  const double speed_mps = vel_world.norm();
  if (speed_mps <= kMinSpeed) return drag;  // No relative wind -> no normal force.
  ...
  const double alpha_rad = std::acos(cos_alpha);
  if (alpha_rad <= kMinAlpha) return drag;  // Essentially aligned -> negligible lift.
  ...
  const double normal_mag_n =
      0.5 * atm.density * speed_mps * speed_mps * cfg_.ref_area * cfg_.cn_alpha * alpha_rad;
  const Vector3 normal = lift_dir * normal_mag_n;
...
Vector3 AeroModel::force6dofHiFi(const Vector3& vel_world, const Quaternion& att,
  ...
  const double speed_mps = vel_world.norm();
  if (speed_mps <= kMinSpeed) return drag;
  ...
  const double alpha_rad = std::acos(cos_alpha);
  if (alpha_rad <= kMinAlpha) return drag;
  ...
  const double mach = atm.speed_of_sound > 0.0 ? speed_mps / atm.speed_of_sound : 0.0;
  const double cn = normalForceCoeff(alpha_rad, mach);
  const double q_bar_pa = 0.5 * atm.density * speed_mps * speed_mps;
  const Vector3 normal = lift_dir * (q_bar_pa * cfg_.ref_area * cn);
  return drag + normal;
}
...
Vector3 AeroModel::momentAero(const Vector3& vel_world, const Quaternion& att,
  ...
  const double mach = atm.speed_of_sound > 0.0 ? speed_mps / atm.speed_of_sound : 0.0;
  const double q_bar_pa = 0.5 * atm.density * speed_mps * speed_mps;
  const double qSd = q_bar_pa * cfg_.ref_area * cfg_.ref_length;  // dynamic-pressure * S * d
  ...
  if (alpha_rad > kMinAlpha) {
    ...
    if (axis_norm > kMinAlpha) {
      ...
      const double cm = pitchMomentCoeff(alpha_rad, mach);
  ...
  const double rate_scale = qSd * cfg_.ref_length / (2.0 * speed_mps);
